...
# """
    ...
    def failure_crushmap(self, n_host, n_disk, fail_disk_num):
        device_id = 0
        host_id = -2
        children = []
        fail_host_num = 0
        if fail_disk_num > n_disk:
          fail_host_num = fail_disk_num / n_disk
          fail_disk_num = fail_disk_num % n_disk
        n_host -= fail_host_num
        for host in range(n_host):
            host_dict = {}
            host_dict["type"] = "host"
            host_dict["name"] = "host" + str(host)
    ...
    def cal_pg_weight(self):
        ...
        for pg, osds in self.pg_to_osds.items():
            ...
            for osd in osds:
                ...
                normalized_pg_num = (len(self.pg_in_osd[osd]) / (self.osd_weight[osd] ** 1 ))
    ...
    def select_pg(self, key):
        ...
        # # entire rendezvous, without "the Power of Two Choices"
        # for pgid in range(self.pg_num):
        #     node = self.pg_Rendezvous_node[pgid]
        #     if node.compute_weighted_score(key) > max_score:
        #         max_score = node.compute_weighted_score(key)
        #         max_pgid = pgid
    ...
    def print_osd_used_capacity(self, osd_id_1, osd_id_2):
        ...
        logger.log("standard deviation: " + str(np.std(list(self.osd_used_capacity.values()))))
...
def origin_weightedPG(b, inode_num, inode_size):
  ...
  # save osd_used_capacity to file
  with open('origin_used_capacity.json', 'w') as f:
      json.dump(b.osd_used_capacity, f)

  # # read osd_used_capacity from file
  # b_osd_used_capacity = {}
  # with open('b_osd_used_capacity70.json', 'r') as f:
  #     b_osd_used_capacity = json.load(f)
  #     b_osd_used_capacity = {int(k): v for k, v in b_osd_used_capacity.items()}
  #     b.osd_used_capacity = b_osd_used_capacity
  #     # print(b.osd_used_capacity)
  
  print("total object number:", inode_num*inode_size)
  print("osd used capacity before failure:")
  # b.print_osd_used_capacity(0, 0)
  return b.osd_used_capacity
  # print("failure disk num:", fail_disk_num)
  # max_after_expan, min_after_expan = new_b.print_osd_used_capacity(0, 0)
  
def weightedPG_compare(b, new_b, inode_num, inode_size, fail_disk_num):
  b.osd_used_capacity.clear()
  new_b.osd_used_capacity.clear()
  # power = 4.5
  b.cal_pg_weight()
  ...
  change_cnt = 0
  # thre = 4
  for thre in range(0, 1):
    new_b.osd_used_capacity.clear()
    print("thre:", thre)
    if thre > 0:
      for pgid in range(b.pg_num):
        if abs(b.pg_to_sum[pgid] - new_b.pg_to_sum[pgid]) <= thre and b.pg_weight[pgid] != new_b.pg_weight[pgid]:
          change_cnt += 1
          new_b.pg_weight[pgid] = b.pg_weight[pgid]
    ...
    # # save osd_used_capacity to file
    # with open('b_osd_used_capacity70.json', 'w') as f:
    #     json.dump(b.osd_used_capacity, f)
        
    # # save osd_used_capacity to file
    # with open('new_b_osd_used_capacity70.json', 'w') as f:
    #     json.dump(new_b.osd_used_capacity, f)
    ...
    #     b.osd_used_capacity = b_osd_used_capacity
    #     # print(b.osd_used_capacity)
    # new_b_osd_used_capacity = {}
    # with open('new_b_osd_used_capacity70.json', 'r') as f:
    #     new_b_osd_used_capacity = json.load(f)
    #     new_b_osd_used_capacity = {int(k): v for k, v in new_b_osd_used_capacity.items()}
...
def failure_test(host_num, disk_num, pg_num, overall_used_capacity, power, type, k, m):
  ...
  # origin_osd_used_capacity = origin_weightedPG(b, inode_num, inode_size)

  # # failure
  # for fail_disk_num in range(1, 2):
  fail_disk_num = b.n_disk
  print("fail disk num:", fail_disk_num)
  new_b = Balancer(host_num, disk_num, pg_num, overall_used_capacity, power, type, k, m)
  new_b.disk_num = new_b.n_host * new_b.n_disk - fail_disk_num
  for osd in range(b.disk_num, new_b.disk_num):
    new_b.osd_weight[osd] = 1
  print("after failure: host num:", new_b.n_host, " disk num:", new_b.n_disk, " fail disk num:", fail_disk_num)
  
  new_crushmap = new_b.failure_crushmap(new_b.n_host, new_b.n_disk, fail_disk_num)
  new_b.c = Crush()
  ...
  # for pgid in range(b.pg_num):
  #   diff = abs(b.pg_to_sum[pgid] - new_b.pg_to_sum[pgid])
  #   diff_map[diff] += 1
  # print("fail disk num:", fail_disk_num)
  # print(dict(diff_map))
  
  # diffs = [abs(b.pg_to_avg[pgid] - new_b.pg_to_avg[pgid]) for pgid in range(b.pg_num)]
  # sorted_diffs = sorted(diffs)
  # threshold_index = int(0.8 * len(sorted_diffs))
  # min_80_percent_diffs = sorted_diffs[:threshold_index]
  # print("fail disk num:", fail_disk_num)
  # print("min 80 percent diffs:", min_80_percent_diffs[0], "-", min_80_percent_diffs[-1])
  
  
  # with open(str(fail_disk_num) + '_pg_to_avg.json', 'w') as f:
  #     for pgid, avg in b.pg_to_avg.items():
  #       f.write(str(pgid) + " " + str(avg) + " " +str(new_b.pg_to_avg[pgid]) + "\n")
  
  
#   weightedPG_compare(b, new_b, inode_num, inode_size, fail_disk_num)
...
for host_num, extra_hosts, disk_num, type, k, m, power in test_cases:
  # for overall_used_capacity in [0.9]:
    ...
    failure_test(total_hosts, disk_num, pg_num, overall_used_capacity, power, type, k, m)
