    ...
    def write_obj(self):
        ...
        while True:
            ...
            for i in range(len(osds)):
                ...
                if used_capacity >= self.disk_size*1024:
                    print("overflow! osd id:", osds[i])
                    print(used_capacity, self.disk_size)
                    print("object num", object_no)
                    disk_capacity = b.disk_num*b.disk_size*1024 #MB
                    if b.crush_type == 'rep':
                        obj_load_num = int(disk_capacity/b.replication_count/b.obj_size)
                    elif b.crush_type == 'ec':
                        redun = (b.k + b.m)/b.k
                        obj_load_num = int(disk_capacity/redun/b.obj_size)
                    eff_cap = object_no / obj_load_num
                    print("obj load num:", obj_load_num)
                    print("effective capacity:", eff_cap)
                    return
                self.osd_used_capacity[osds[i]] = used_capacity
            object_no += 1
