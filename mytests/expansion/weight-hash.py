...
# """
    ...
    def crush_pg_to_osds(self):
        ...
        with open('weight-hash-osdmap.json', 'w') as f:
            json.dump(self.pg_to_osds, f)
    ...
    def ideal_weight(self, overall_obj_num):
      ...
      for pg in range(self.pg_num):
          ...
          for i in range(len(osds)):
              used_capacity = float(osd_usage.get(osds[i], 0)) * 128 * 1024
              used_capacity += plus_capacity
              # if used_capacity >= self.disk_size*1024:
                  # print("overflow! osd id:", osds[i])
              osd_usage[osds[i]] = used_capacity / (128 * 1024)
    ...
    def cal_pg_weight(self, power):
        ...
        with open('weight-hash-pg-weight.json', 'w') as f:
    ...
    def print_osd_used_capacity(self, max_osd_id, min_osd_id):
        ...
        osd_usage = [value / (self.disk_size * 1024) for value in self.osd_used_capacity.values()]
        std = -np.std(osd_usage)
