...
def raw_crush_compare(b, new_b, inode_num, inode_size):
  ...
  # #TODO: rewrite some objects to test the new balance rate
...
def weightedPG_compare(b, new_b, inode_num, inode_size):
  ...
  #   test_ideal_weight(new_b, inode_num*inode_size, max_after_expan, min_after_expan)
  # return
  
  # for power in range(8, 10):
  power = 4
  print("power:", power)
  # new_b.osd_used_capacity = new_b_osd_used_capacity
  new_b.cal_pg_weight(power)
  ...
  # with open('obj_to_normal.json', 'w') as f:
  #   json.dump(obj_to_pg, f)
...
def expansion_test(host_num, disk_num, pg_num, overall_used_capacity, power, type, k, m):
  ...
  # diff_osds_cnt = 0
  # for pg in range(b.pg_num):
  #   if b.pg_to_osds[pg] != new_b.pg_to_osds[pg]:
  #     diff_osds_cnt += 1
  #     print("pg id:", pg, " osds:", b.pg_to_osds[pg], " new osds:", new_b.pg_to_osds[pg])
  # print("different osds count:", diff_osds_cnt)
...
# print("hash type: 1 : raw crush compare, 2 : weighted PG compare")
# hash_type = input("Enter the hash type: ")
# expansion_test(hash_type)
