...
# """
        ...
        with open('new_b_osd_used_capacity70.json', 'r') as f:
            ...
            osd_used_capacity = {int(k): v for k, v in osd_used_capacity.items()}
        ...
        for i in range(self.n_host):
          for j in range(self.n_disk):
            self.osd_to_capacity[osd_id] = 128 * 1024 - osd_used_capacity[osd_id]
            self
            if self.osd_to_capacity[osd_id] < min_osd_capacity:
              min_osd_capacity = self.osd_to_capacity[osd_id]
            osd_id += 1
            
        # print(self.osd_to_capacity)
        print("min osd capacity:", min_osd_capacity)
        print("max osd capacity:", max(self.osd_to_capacity.values()))
            
        self.overall_capacity = sum([v for _, v in self.osd_to_capacity.items()]) / 1024
        self.osd_weight = {}
    ...
    def ideal_weight(self, overall_obj_num):
      self.cal_pg_weight(4)
      # print(self.pg_weight)
      overall_weight = sum(self.pg_weight.values())
      osd_usage = {}
      write_obj_num = 0
      for pg in range(self.pg_num):
          weight = self.pg_weight[pg]
          obj_num = overall_obj_num * weight / overall_weight
          write_obj_num += obj_num
          osds = self.pg_to_osds.get(pg, [])
          plus_capacity = 0
          if self.crush_type == 'ec':
              plus_capacity = (obj_num * self.obj_size / self.k)
          elif self.crush_type == 'rep':
              plus_capacity = (obj_num *  self.obj_size)
          for osd in osds:
              used_capacity = float(osd_usage.get(osd, 0)) * self.osd_to_capacity[osd]
              used_capacity += plus_capacity
              # if used_capacity >= self.disk_size*1024:
                  # print("overflow! osd id:", osds[i])
              osd_usage[osd] = used_capacity / self.osd_to_capacity[osd]
      
      print("write obj num:", write_obj_num)
      max_used_capacity = 0
      max_osdid = 0
      min_used_capacity = sys.maxsize
      min_osdid = 0
      obj_sum = 0
      for osd, used in osd_usage.items():
          obj_sum += used
          ...
          if used <= min_used_capacity:
              ...
              min_osdid = osd
      avg_capacity = self.overall_used_capacity
      # print("MAX/MIN used capacity:" + str(float(max_used_capacity))+ "/" + str(float(min_used_capacity)))
      ...
      print("MAX/MIN used capacity deviation:" + str(max_used_capacity/avg_capacity)+ "/" + str(min_used_capacity/avg_capacity))
      ...
      # print("MAX/MIN used osd id:" + str(max_osdid)+ "/" + str(min_osdid))
      ...
      std = np.std(list(osd_usage.values()))
    ...
    def print_osd_used_capacity(self, max_osd_id, min_osd_id):
        ...
        for osd, used in self.osd_used_capacity.items():
            obj_sum += used
            curr_used_percent = float(used) / (self.osd_to_capacity[osd])
