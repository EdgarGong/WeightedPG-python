...
static int parse_bucket(LibCrush *self, PyObject *bucket, int *idout, int *weightout, PyObject *trace)
{
  ...
  if (children != NULL) {
    for (pos = 0; pos < PyList_Size(children); pos++) {
      ...
      // printf("crush_bucket_add_item==%d\n", child);
      if (r < 0) {
...
LibCrush_parse(LibCrush *self, PyObject *args)
{
  ...
  self->crush_wrapper.crush = self->map;
  ...
  for (i = 0; i < result_len; i++) {
    ...
    if (result[i] == CRUSH_ITEM_NONE) {
      python_result = Py_None;
        ...
        python_result = MyInt_FromInt(result[i]);
        if (PyErr_Occurred())
            return 0;
...
static inline unsigned ceph_str_hash_rjenkins(const char *str, unsigned length)
{
...
static PyObject *
LibCrush_ceph_str_hash_rjenkins(LibCrush *self, PyObject *args)
{
    const char* str;
    unsigned int len;

    if (!PyArg_ParseTuple(args, "sI", &str, &len))
        return 0;

    unsigned int res = ceph_str_hash_rjenkins(str, len);
    PyObject *value = Py_BuildValue("i", res);
    return value;
}
...
try_pg_upmap(
    ...
    const std::vector<int>& more_underfull,  ///< more osds only slightly underfull
    std::vector<int> *orig,
    std::vector<int> *out) {
  ...
  int r = self->crush_wrapper.try_remap_rule(
    cct,
    ruleno,
    replication_count,
    overfull, underfull,
...
static PyObject *
LibCrush_try_pg_upmap(LibCrush *self, PyObject *args) {
    int pg, replication_count;
    PyObject *rule = NULL, *overfull = NULL, *underfull = NULL, *more_underfull = NULL, *orig = NULL, *out = NULL;
    if (!PyArg_ParseTuple(args, "iiOOOOOO", &pg, &replication_count, &rule, &overfull, &underfull, &more_underfull, &orig, &out))
        return 0;
  ...
  if (PyErr_Occurred())
    ...
    std::set<int> overfull_c;
    std::vector<int> underfull_c, more_underfull_c, orig_c, out_c;

    for (int i = 0; i < PyList_Size(overfull); ++i) {
        int item = (int) PyInt_AsLong(PyList_GetItem(overfull, i));
        overfull_c.insert(item);
    }
    for (int i = 0; i < PyList_Size(underfull); ++i) {
        int item = (int) PyInt_AsLong(PyList_GetItem(underfull, i));
        underfull_c.push_back(item);
    }
    for (int i = 0; i < PyList_Size(more_underfull); ++i) {
        int item = (int) PyInt_AsLong(PyList_GetItem(more_underfull, i));
        more_underfull_c.push_back(item);
    }
    for (int i = 0; i < PyList_Size(orig); ++i) {
        int item = (int) PyInt_AsLong(PyList_GetItem(orig, i));
        orig_c.push_back(item);
    }

    int r = try_pg_upmap(self, ruleno, replication_count, pg, overfull_c, underfull_c, more_underfull_c, &orig_c, &out_c);

    for (int i = 0; i < out_c.size(); ++i) {
        PyList_Append(out, PyLong_FromLong(out_c[i]));
    }
    if (r < 0)
    return 0;
  Py_RETURN_TRUE;
...
//            PyDoc_STR("write to Ceph txt/bin/json crushmap") },
    ...
    { "try_pg_upmap", (PyCFunction) LibCrush_try_pg_upmap, METH_VARARGS,
            PyDoc_STR("try pg upmap")},
    { "ceph_str_hash_rjenkins",  (PyCFunction) LibCrush_ceph_str_hash_rjenkins, METH_VARARGS,
            PyDoc_STR("ceph_str_hash_rjenkins")},
    { NULL }
};
