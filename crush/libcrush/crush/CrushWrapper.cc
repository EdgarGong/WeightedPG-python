					...
					bool exclude_shadow) const
{
  ...
  // std::cout << "get_children_of_type " << b->type << type << std::endl;
  if (IS_ERR(b)) {
    return;
  }
  if (b->type < type) {
    // give up
    // std::cout << "get_children_of_type ret1" << std::endl;
    return;
  } else if (b->type == type) {
    if (!is_shadow_item(b->id) || !exclude_shadow) {
      children->insert(b->id);
    }
    // std::cout << "get_children_of_type ret2" << std::endl;
    return;
  }
  for (unsigned n = 0; n < b->size; n++) {
    get_children_of_type(b->items[n], type, children, exclude_shadow);
  }
  // std::cout << "get_children_of_type ret3" << std::endl;
}
...
int CrushWrapper::get_parent_of_type(int item, int type, int rule) const
{
  if (rule < 0) {
    ...
    do {
      ...
      if (r < 0) {
        // std::cout << "[get_parent_of_type] return 0 r==" << r << std::endl;
        return 0;
      }
    } while (get_bucket_type(item) != type);
    // std::cout << "[get_parent_of_type] return item==" << item << std::endl;
    return item;
  }
  ...
  for (auto root : roots) {
    // std::cout << "[get_parent_of_type] traverse root " << root << std::endl;
    set<int> candidates;
    get_children_of_type(root, type, &candidates, false);
    for (auto candidate : candidates) {
      // std::cout << "[get_parent_of_type] traverse candidate " << candidate << std::endl;
	...
	// which should generally be true.
        // std::cout << "[get_parent_of_type] return candidate " << candidate << std::endl;
        return candidate;
      }
    }
  }

  // std::cout << "[get_parent_of_type] return 0" << std::endl;
  return 0; // not found
}
...
int CrushWrapper::get_osd_pool_default_crush_replicated_ruleset(CephContext *cct)
{
  ...
  if (crush_ruleset < 0) {
    crush_ruleset = find_first_ruleset(1);
  ...
  int rule) const
{
 ...
 // std::cout << "[_choose_type_stack]" << " stack " << stack
		//  << " orig " << orig
		//  << " at " << *i
		//  << " pw " << *pw
		//  << std::endl;
 ...
 // std::cout << "[_choose_type_stack]" << " cumulative_fanout " << cumulative_fanout
		//  << std::endl;
  ...
  for (auto osd : underfull) {
    ...
    for (int j = (int)stack.size() - 2; j >= 0; --j) {
     ...
     // std::cout << "[_choose_type_stack]" << " underfull " << osd << " type " << type
		    //  << " is " << item << std::endl;
      if (!subtree_contains(root_bucket, item)) {
       // std::cout << "[_choose_type_stack]" << " not in root subtree " << root_bucket << std::endl;
        continue;
      }
      underfull_buckets[j].insert(item);
    }
  }
 // std::cout << "[_choose_type_stack]" << " underfull_buckets " << underfull_buckets << std::endl;

  for (unsigned j = 0; j < stack.size(); ++j) {
    int type = stack[j].first;
    int fanout = stack[j].second;
    int cum_fanout = cumulative_fanout[j];
   // std::cout << " level " << j << ": type " << type << " fanout " << fanout
		  //  << " cumulative " << cum_fanout
		  //  << " w " << w << std::endl;
    vector<int> o;
    auto tmpi = i;
    if (i == orig.end()) {
     // std::cout << "[_choose_type_stack]" << " end of orig, break 0" << std::endl;
      break;
    }
    for (auto from : w) {
	...
	if (type > 0) {
	  ...
	  // std::cout << "[_choose_type_stack]" << "   from " << *tmpi << " got " << item
			//  << " of type " << type << " over leaves " << leaves[pos] << std::endl;
	} else {
	  // leaf
	  bool replaced = false;
	  if (overfull.count(*i)) {
	    for (auto item : underfull) {
	      // std::cout << "[_choose_type_stack]" << " pos " << pos
			    //  << " was " << *i << " considering " << item
			    //  << std::endl;
	      if (used.count(item)) {
		// std::cout << "[_choose_type_stack]" << "   in used " << used << std::endl;
		continue;
	      }
	      if (!subtree_contains(from, item)) {
		// std::cout << "[_choose_type_stack]" << "   not in subtree " << from << std::endl;
		continue;
	      }
	      if (std::find(orig.begin(), orig.end(), item) != orig.end()) {
		// std::cout << "[_choose_type_stack]" << "   in orig " << orig << std::endl;
		continue;
	      }
	      o.push_back(item);
	      used.insert(item);
	      // std::cout << "[_choose_type_stack]" << " pos " << pos << " replace "
			    //  << *i << " -> " << item << std::endl;
	      replaced = true;
              assert(i != orig.end());
	      ++i;
	      ...
	      for (auto item : more_underfull) {
	        // std::cout << "[_choose_type_stack]" << " more underfull pos " << pos
			      //  << " was " << *i << " considering " << item
			      //  << std::endl;
	        if (used.count(item)) {
		  // std::cout << "[_choose_type_stack]" << "   in used " << used << std::endl;
	  ...
	  if (!replaced) {
	    // std::cout << "[_choose_type_stack]" << " pos " << pos << " keep " << *i
			  //  << std::endl;
            assert(i != orig.end());
	    o.push_back(*i);
	    ++i;
	  ...
	  if (i == orig.end()) {
	    // std::cout << "[_choose_type_stack]" << " end of orig, break 1" << std::endl;
	    break;
	  }
	}
      }
	...
	for (int pos = 0; pos < fanout; ++pos) {
	  if (underfull_buckets[j].count(o[pos]) == 0) {
	    ...
	    if (any_overfull) {
	      // std::cout << " bucket " << o[pos] << " has no underfull targets and "
		...
		if (std::find(o.begin(), o.end(), alt) == o.end()) {
		  ...
		  if (j == 0 ||
		    ...
		    // std::cout << "  alt " << alt << " for " << o[pos]
				  //  << " has different parent, skipping" << std::endl;
		  }
		}
	      }
	    }
      ...
      if (i == orig.end()) {
	// std::cout << "[_choose_type_stack]" << " end of orig, break 2" << std::endl;
	break;
      }
    }
    // std::cout << "[_choose_type_stack]" << "  w <- " << o << " was " << w << std::endl;
    w.swap(o);
  }
  *pw = w;
  return 0;
...
{
  // std::cout << "try_remap_rule start" << std::endl;
  const crush_map *map = crush;
  const crush_rule *rule = get_rule(ruleno);
  assert(rule);

 // std::cout << "[try_remap_rule]" << " ruleno " << ruleno
  ...
  for (unsigned step = 0; step < rule->len; ++step) {
    ...
    // std::cout << "[try_remap_rule]" << " step " << step << " w " << w << std::endl;
    switch (curstep->op) {
    case CRUSH_RULE_TAKE:
	...
	// std::cout << "[try_remap_rule]" << " take " << w << std::endl;
      } else {
	// std::cout << " bad take value " << curstep->arg1 << std::endl;
      }
      break;
	...
	if (r < 0) {// std::cout << "try_remap_rule return first r " << r << std::endl;
	  return r;}
	type_stack.clear();
      }
      break;
    ...
    case CRUSH_RULE_EMIT:
      // std::cout << " emit " << w << std::endl;
      if (!type_stack.empty()) {
	int r = _choose_type_stack(cct, type_stack, overfull, underfull, more_underfull, orig,
				   i, used, &w, root_bucket, ruleno);
	if (r < 0) {// std::cout << "try_remap_rule return second r " << r << std::endl;
	  return r;}
	type_stack.clear();
      }
      for (auto item : w) {
  ...
  // std::cout << "try_remap_rule return 0" << std::endl;
  return 0;
}
